...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    // above corresponds to endpoint "https://localhost:8443/api/products/search/findBynameContaining?name=somthing"
...
//    And since we have this custom method, spring data rest will not provide auto created endpoints. we have to define endpoints as well in controller.But we need to define only the endpoint. Though spring data rest does not give an automatically created endpoint, it still understands what we are trying to do by method name. so you dont need to manually define this method anywhere. spring data rest will understand and run the suitable sql query in db and will fetch the required data. Its just that it will not provide us with auto-created endpoint(like in above 2 cases) , which we have to define in controller.
